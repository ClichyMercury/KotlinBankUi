...
import com.finsim.presentation.components.Sparkline
...
fun MarketScreen(
    state: MarketUiState,
    onRefresh: () -> Unit,
    onSelectType: (AssetType?) -> Unit,
    onAssetClick: (Uuid) -> Unit,
    ...
    Scaffold(
        ...
        when {
            state.isLoading -> LoadingScreen(modifier = Modifier.padding(padding))
                ...
                item { TypeFilterRow(selected = state.selectedType, onSelect = onSelectType) }
                ...
                if (state.assets.isEmpty()) {
                    ...
                    items(state.assets, key = { it.id.toString() }) { asset ->
                        MarketAssetRow(
                            asset = asset,
